...
var acym_helperCampaigns = {
  ...
  setDisplayNotif: function setDisplayNotif(message, type) {
  ...
  resetThePopup: function resetThePopup() {
    jQuery('.rs_date_field').on('click', function () {
      ...
      if ($input.val().indexOf('[time]') !== -1) {
        ...
        $modal.find('[data-type="specific"]').trigger('click'); //We convert the unix time to a format the lib can understand
...
var acym_helperEditorWysid = {
  ...
  setColumnRefreshUiWYSID: function setColumnRefreshUiWYSID() {
    var autoSave = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    var initEdit = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    ...
    acym_editorWysidTinymce.checkForEmptyText(); // This was breaking the zone resizing, I kept it because I don't know why Rémi added it the 28/01/20
  ...
  resizeBlockContainers: function resizeBlockContainers() {
    jQuery('.acym__wysid__column tbody').each(function () {
      if (jQuery(this).children().length === 0) {
        jQuery(this).css({
          'min-height': '75px',
          'display': 'block'
        }).closest('table').css('min-height', '75px').css('display', 'block');
      } else {
        jQuery(this).css({
          'min-height': '0px',
  ...
  addDefaultZone: function addDefaultZone() {
    var $wysidRows = jQuery('.acym__wysid__row'); // For each zone in the editor, if it is empty we add an empty container where we can drop blocks

    var defaultRowAdded = false;
  ...
  addDefaultBlock: function addDefaultBlock() {
    var $wysidRows = jQuery('.acym__wysid__row'); // Should always be 1, this is the td containing the zones

    if ($wysidRows.length !== 1) return;
    var $columns = $wysidRows.find('.acym__wysid__column'); // If the container contains only one empty block, replace it with the default "drop here" text
  ...
  resizeEditorBasedOnPage: function resizeEditorBasedOnPage() {
    var adminHeight;
    ...
    var emailContainerHeight = jQuery(window).height() - (wrapperPaddingTop + adminHeight + toolbarTop + 64);
...
var acym_helperFile = {
  ...
  setSwitchView: function setSwitchView() {
...
var acym_helper = {
  ...
  setButtonRadio: function setButtonRadio() {
    jQuery('.button-radio').off('click').on('click', function (event) {
      ...
      var callback = jQuery(this).attr('acym-callback');
      setTimeout(function () {
...
var acym_helperInput = {
  ...
  setMulticouple: function setMulticouple() {
    jQuery('.multikeyvalue_container_new').off('click').on('click', function (event) {
      event.preventDefault();
      jQuery(this).before('<div class="multikeyvalue_container_separator cell small-6"></div>' + '<input type="text" class="cell" placeholder="' + ACYM_JS_TXT.ACYM_DKIM_KEY + '" value=""/>' + '<input type="text" class="cell" placeholder="' + ACYM_JS_TXT.ACYM_VALUE + '" value="" />');
      acym_helperInput.setNewMulticouple();
    });
    acym_helperInput.setNewMulticouple();
  },
  setNewMulticouple: function setNewMulticouple() {
    jQuery('.multikeyvalue_container input[type="text"]').off('change').on('change', function () {
      var values = {};
      var inputs = jQuery('.multikeyvalue_container input[type="text"]');
      inputs.each(function (index) {
        if (index % 2 === 1) return;
        var key = jQuery(this).val();
        var value = inputs[index + 1].value;
...
var acym_helperListing = {
  ...
  setSelectActions: function setSelectActions() {
    ...
    function triggerAction() {
      ...
      switch (action) {
        case 'delete':
          if (fastAction) {
            confirmMessage = "".concat(ACYM_JS_TXT.ACYM_ARE_YOU_SURE_DELETE_ONE);
          } else {
      ...
      if ('duplicate' === action || 'duplicateFollowup' === action || 'export' === action || acym_helper.confirm(confirmMessage)) {
        ...
        if (fastAction) {
          ...
          var checkboxId = '#checkbox_' + this.dataset.acyElementid;
  ...
  checkFiltersApplied: function checkFiltersApplied() {
    var nbChecked = jQuery('[name="elements_checked[]"]:checked').length;
    var $nbToExport = jQuery('#acym__users__listing__number_to_export');

    if (nbChecked === 0 && (jQuery('#users_list').val() > 0 || jQuery('[name="users_search"]').val().length > 0 || jQuery('.acym__status__select').attr('acym-data-status').length > 0)) {
      $nbToExport.hide();
    } else {
      $nbToExport.show();
    }
  ...
  chooseOneTemplate: function chooseOneTemplate() {
    jQuery('.acym__templates__oneTpl').off('click').on('click', function () {
      ...
      jQuery.post(ajaxUrl, function (res) {
        ...
        $actionContainer.find('.acym__automation__action__mail__name').html(res.data.newMail.name + '<i class="cursor-pointer acymicon-close acym__color__red acym__automation__action__reset__mail margin-left-1"></i>');
...
var acym_helperSelectionMultilingual = {
  ...
  changeLanguage_list: function changeLanguage_list(code) {
    ...
    var frontLabel = code === this.mainLanguage ? this.mainParams.display_name : this.translation[this.currentLanguage].display_name;
  ...
  updateTranslation_list: function updateTranslation_list() {
    jQuery('[name="list[name]"], [name="list[description]"], [name="list[display_name]"]').off('keyup').on('keyup', function () {
      ...
      var column = name.substring(name.indexOf('[') + 1, name.lastIndexOf(']'));
...
var acym_helperSwitch = {
  ...
  setButtonSwitch: function setButtonSwitch() {
    var $switchButtons = jQuery('button[acym-button-switch-type]');
    $switchButtons.off('click').on('click', function (event) {
      event.preventDefault();
      var $clickedType = jQuery(this);
...
var acym_helperToggle = {
  ...
  setToggleGlobal: function setToggleGlobal() {
    jQuery('.acym_toggleable').off('click').on('click', function () {
      ...
      acym_helper.get(url).then(function (res) {
        if (res.error) {
  ...
  setShowMore: function setShowMore() {
    var $showMore = jQuery('.acym__configuration__showmore-head');
    ...
    $selectToggle.on('change', function () {
      ...
      jQuery.each(association, function (value, element) {
        if (currentValue === value) {
          var classes = association[value].split(',');
...
var acym_helperTooltip = {
  ...
  setUserInfoLoadingHover: function setUserInfoLoadingHover() {
    jQuery('.acym__hover__user_info').off('mouseenter').on('mouseenter', function () {
      var _this3 = this;

      if (jQuery(this).find('.acymicon-spin').length > 0) {
        var ajaxUrl = ACYM_AJAX_URL + '&ctrl=users&task=getUserInfo';
        var data = {
        ...
        acym_helper.get(ajaxUrl, data).then(function (response) {
          ...
          if (!acym_helper.empty(response.message)) {
            ...
            for (var _i = 0, _Object$entries = Object.entries(response.data); _i < _Object$entries.length; _i++) {
              ...
              content += "".concat(name, ": ").concat(value, "<br>");
            }
          }

          jQuery(_this3).find('.acym__tooltip__text').html(content);
        });
      }
    });
  }
