...
var acym_helperFilter = {
  setFieldValue: function setFieldValue($field, value) {
    if ($field.hasClass('acym_select2_ajax')) {
      ...
      var numDelayField = $field.attr('id').substr(8);
  ...
  setAutomationReload: function setAutomationReload() {
    jQuery('[acym-automation-reload]').each(function () {
      jQuery(this).on('change', function () {
        var selectReload = jQuery(this);
        setTimeout(function () {
          var params = selectReload.attr('acym-automation-reload');
          var decodedParams = acym_helper.parseJson(params);
          ...
          decodedParams.forEach(function (oneDecodedParams) {
            ...
            if (oneDecodedParams['params']) {
              for (var _i = 0, _Object$entries = Object.entries(oneDecodedParams['params']); _i < _Object$entries.length; _i++) {
            ...
            if (oneDecodedParams['paramFields']) {
              for (var _i2 = 0, _Object$entries2 = Object.entries(oneDecodedParams['paramFields']); _i2 < _Object$entries2.length; _i2++) {
                var _Object$entries2$_i = _slicedToArray(_Object$entries2[_i2], 2),
                    _identifier = _Object$entries2$_i[0],
                    _name = _Object$entries2$_i[1];

                postParams[_identifier] = jQuery('[name="' + _name + '"]').val();
              }
            }
            ...
            jQuery.ajax({
              ...
              success: function success(result) {
                var $container = jQuery(oneDecodedParams['change']);
                $container.html(result);
                ['segments', 'automation'].forEach(function (selector) {
                  var containerClasses = '.acym__' + selector + '__one__filter.acym__' + selector + '__one__filter__classic';
                  var inputClasses = '.acym__' + selector + '__inserted__filter input';
                  inputClasses += ', .acym__' + selector + '__inserted__filter textarea';
                  inputClasses += ', .acym__' + selector + '__inserted__filter select';
                  $container.closest(containerClasses).find(inputClasses).on('change acym-reloaded', function () {
  ...
  setToggle: function setToggle($toggle) {
    var parentClass = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'acym__automation__one__filter';
    if ($toggle.length < 1) return true;
    var paramsToggle = $toggle.attr('data-acym-toggle-filter');
    if (undefined === paramsToggle) return true;
    paramsToggle = acym_helper.parseJson(paramsToggle);
    paramsToggle.forEach(function (paramToggle) {
      var $blockToToggle = $toggle.closest(".".concat(parentClass)).find(".".concat(paramToggle["class"]));
  ...
  toggleBlock: function toggleBlock($toggle, $blockToToggle, paramsToggle) {
    var $input = $blockToToggle.find('[name*="[filters]"]');

    if (paramsToggle.values.includes($toggle.val())) {
      if ($blockToToggle.is(':visible')) return;
      $blockToToggle.css('display', 'flex');
      $input.attr('name', "acym_action".concat($input.attr('name')));
      return;
    }
...
var acym_helperJoomla = {
  ...
  setJoomlaLeftMenu: function setJoomlaLeftMenu() {
    ...
    jQuery('#acym__joomla__left-menu--show').off('click').on('click', function () {
      var $navButton = jQuery('.btn-navbar');
...
var acym_helperMailer = {
  ...
  setButtonCopyFromPlugin: function setButtonCopyFromPlugin() {
    jQuery('.acym__configuration__copy__mail__settings').off('click').on('click', function () {
      ...
      acym_helper.post(ajaxUrl, data).then(function (response) {
        if (response.error) {
  ...
  setSynchroExistingUsers: function setSynchroExistingUsers() {
    jQuery('.acym__configuration__sending__synch__users').off('click').on('click', function () {
      var sendingMethod = this.getAttribute('sending-method-id');
      if (sendingMethod === undefined) return false;
      var $icon = jQuery(this).closest('.acym__sending__methods__synch').find('.acym__configuration__sending__method-icon');
      var $message = jQuery(this).closest('.acym__sending__methods__synch').find('.acym__configuration__sending__method-synch__message');
      $icon.removeClass('acym__color__red acym__color__green').addClass('acymicon-circle-o-notch acymicon-spin');
      $message.html('');
      var data = {
        ctrl: 'configuration',
  ...
  dislayAuth2Params: function dislayAuth2Params() {
    var auth2Smtp = ['smtp.gmail.com', 'smtp-mail.outlook.com', 'smtp.office365.com'];
    var outlookSmtp = ['smtp-mail.outlook.com', 'smtp.office365.com'];
    var smtpHost = jQuery('#smtp_host');
    var smtpHostValue = smtpHost.val().toLowerCase().trim();
    var oauth2SendingParams = jQuery('.acym__oauth2_sending_params');
    var defaultSendingParams = jQuery('.acym__default_auth_sending_params');
    var tenantContainer = jQuery('#smtp_tenant_container');
...
var acym_helperNotification = {
  ...
  readFullNotification: function readFullNotification() {
    jQuery('.acym__header__notification__one').off('click').on('click', function () {
      var $messageTag = jQuery(this).find('.acym__header__notification__message');
      var messageInAttribute = $messageTag.attr('data-acym-full');
      if (acym_helper.empty(messageInAttribute)) return true;
      var messageInTag = $messageTag.html();
...
var acym_helperSegment = {
  ...
  reloadCounters: function reloadCounters(element) {
    ...
    var ajaxData = jQuery(element).closest('#acym_form').serialize() + '&page=acymailing_segments&ctrl=segments&task=countResults&and=' + and + '&or=' + or;
    this.ajaxCalls[and] = jQuery.post(ajaxUrl, ajaxData).done(function (response) {
      response = acym_helper.parseJson(response);
      jQuery('#results_' + and).find('.acym__segments__edit__filter-results').html(response.error ? ACYM_JS_TXT.ACYM_ERROR : response.message);
    }).fail(function () {
  ...
  reloadGlobalCounter: function reloadGlobalCounter() {
    ...
    var ajaxData = groupFilter.closest('#acym_form').serialize() + '&page=acymailing_segments&ctrl=segments&task=countResultsTotal';
    $counterInput.html('<i class="acymicon-circle-o-notch acymicon-spin"></i>');
    this.globalAjaxCall = jQuery.post(ajaxUrlTotal, ajaxData).done(function (response) {
      response = acym_helper.parseJson(response);
  ...
  setSelectFilters: function setSelectFilters(type) {
    ...
    var filters = acym_helper.parseJson($options.val());
    jQuery('.acym__segments__select__' + type + '__filter').off('change').on('change', function () {
      ...
      if (jQuery(this).val() == 0) {
        ...
        acym_helperSegment.reloadCounters(jQuery(this).closest('.acym__segments__one__filter.acym__segments__one__filter__classic').find('.acym__segments__inserted__filter input, .acym__segments__inserted__filter select, .acym__segments__inserted__filter textarea'));
      }

      var $toggle = jQuery(this).closest('.acym__segments__one__filter').find('[data-acym-toggle-filter]');
  ...
  rebuildFilters: function rebuildFilters() {
    ...
    var filters = acym_helper.parseJson($filterElement.val());
    ...
    jQuery.each(filters, function (numOR, oneORBlock) {
      ...
      jQuery.each(oneORBlock, function (numAND, oneFilter) {
        ...
        if (and !== 0) {
          jQuery('.acym__segments__group__filter[data-filter-number="' + or + '"]').find('.acym__segments__add-filter[data-filter-type]').trigger('click');
        }
